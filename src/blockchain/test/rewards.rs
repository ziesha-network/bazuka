...
fn test_correct_rewards() {
    ...
    let expected_validator_reward_1 = Amount::new(expected_reward_1.value * 12 / 255);
    assert_eq!(expected_reward_1, Amount::new(19999999999999));
    let draft = chain
    ...
    let validator_stake = Amount::new(25);
    let delegator_stake = Amount::new(75);

    let expected_reward_2 = chain.next_reward().unwrap();
    assert_eq!(expected_reward_2, Amount::new(19999799999999));
    let expected_validator_reward_2 = Amount::new(expected_reward_2.value * 12 / 255);
    let draft = chain
        .draft_block(
            0,
            &[
    ...
    let expected_delegator_balance_2 = Amount::new(10000) - delegator_stake;
    assert!(close_enough(
        chain
            .get_balance(validator.get_address(), TokenId::Ziesha)
            .unwrap(),
    ...
    assert_eq!(expected_reward_3, Amount::new(19999600001999));
    let expected_validator_reward_3 = Amount::new(expected_reward_3.value * 12 / 255);
    let draft = chain
        .draft_block(0, &[], &validator, true)
        .unwrap()
        .unwrap();
    ...
    assert!(close_enough(
        chain
            ...
            - expected_validator_balance_2,
        Amount::new(
            (expected_reward_3 - expected_validator_reward_3).value / 5
                + expected_validator_reward_3.value
        )
    ));
    assert!(close_enough(
        chain
            ...
            .unwrap()
            - expected_delegator_balance_2,
        Amount::new((expected_reward_3 - expected_validator_reward_3).value * 3 / 5)
    ));
}
